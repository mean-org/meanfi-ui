...
const AlertIcon = ({ validationStatus }: Props) => {
  ...
  return (
    <span aria-label={iconName} className={`anticon anticon-${iconName} ant-alert-icon`}>
      {icon}
    </span>
  );
};
  ...
  return (
    <div data-show='true' className={`ant-alert ant-alert-${validationStatus.severity} translucent`} role='alert'>
      ...
      <div className='ant-alert-content'>
        {alertTitle ? <div className='ant-alert-message'>{alertTitle}</div> : null}
        {alertMessage ? (
          <div className='ant-alert-description' style={{ display: 'block' }}>
            {alertMessage}
          </div>
        ) : null}
      </div>
