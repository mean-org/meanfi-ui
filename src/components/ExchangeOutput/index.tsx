...
export const ExchangeOutput = (props: {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const exchangeInfos = props.clients.filter(c => c.exchange).map(c => c.exchange);

      const firstInfo = exchangeInfos[0];
      const lastInfo = exchangeInfos[exchangeInfos.length - 1];
      const fromAmount = Number.parseFloat(props.fromTokenAmount);
      ...
      <div className='flex-fixed-right'>
            ...
            >
              {`(~$${
                props.toToken && props.toTokenBalance
                  ? formatAmount(
                      Number.parseFloat(props.toTokenBalance) * getTokenPriceByAddress(props.toToken.address),
                      2,
                    )
                  : '0.00'
              })`}
        ...
        <div className='right inner-label'>
          ...
          >
            ~$
            {props.toToken && props.toTokenBalance
              ? formatAmount(Number.parseFloat(props.toTokenBalance) * getTokenPriceByAddress(props.toToken.address), 2)
              : '0.00'}
          </span>
        </div>
      </div>
