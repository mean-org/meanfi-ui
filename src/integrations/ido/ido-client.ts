...
const IDO_READONLY_PUBKEY = new PublicKey(
  '3KmMEv7A8R3MMhScQceXBQe69qLmnFfxSM3q8HyzkrSx',
);
const MSP_FEE_PUBKEY = new PublicKey(MSP_FEE_TREASURY);
const MSP_PROGRAM_PUBKEY_DEVNET = new PublicKey(
  '9yMq7x4LstWYWi14pr8BEBsEX33L3HnugpiM2PT96x4k',
);
const MSP_PROGRAM_PUBKEY_MAINNET = new PublicKey(
...
export class IdoClient {
  ...
  constructor(
    ...
    if (!rpcUrl) throw new Error('wallet cannot be null or undefined');

    this.userPubKey = userPubKey;
    this.rpcUrl = rpcUrl;
    const readonlyWallet = IdoClient.createReadonlyWallet(IDO_READONLY_PUBKEY);
    this.readonlyProgram = IdoClient.createProgram(
      rpcUrl,
      readonlyWallet,
      confirmOptions,
    );
  ...
  private static createProgram(
    ...
    wallet: Wallet,
    confirmOptions?: anchor.web3.ConfirmOptions,
  ): anchor.Program<MeanIdoPool> {
    const provider = IdoClient.getAnchorProvider(
      rpcUrl,
      wallet,
      confirmOptions,
    );
    const programId = new anchor.web3.PublicKey(ido_idl.metadata.address);
    const program = new anchor.Program<MeanIdoPool>(
      ido_idl as any,
      programId,
      provider,
    );
  ...
  public async findUserIdoProgramAddress(
    userPubKey: PublicKey,
    meanIdoPubKey: PublicKey,
  ...
  public async createInitializeIdoTx(
    ...
    const program = IdoClient.createProgram(
      this.rpcUrl,
      authorityWallet,
      this.readonlyProvider.opts,
    );

    // TODO: params check
    if (idoName.length === 0) throw Error('Invalid IDO name');
    if (idoName.length > 10)
      throw Error('IDO name is too long. Max lenght: 10');
    if (idoMeanAmount <= 0) throw Error('Invalid MEAN amount for IDO');
    ...
    if (idoTimes.redeemEndTs.lt(idoTimes.redeemStartTs))
      throw Error(
        "Invalid IDO times: 'redeem end' must be after 'redeem start'",
      );

    const [idoAccount, idoAccountBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [idoAuthority.toBuffer(), Buffer.from(idoName)],
        program.programId,
      );

    const [meanPool, meanPoolBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [idoAccount.toBuffer(), Buffer.from('mean_pool')],
        program.programId,
      );

    const [usdcPool, usdcPoolBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [idoAccount.toBuffer(), Buffer.from('usdc_pool')],
        this.readonlyProgram.programId,
      );
    ...
    if (!meanMintSupply) throw new Error('MEAN MINT not found');
    if (meanMintSupply.value.decimals !== DECIMALS)
      throw Error(
        `Unsupported MEAN decimals: ${meanMintSupply.value.decimals}. Only value supported is: ${DECIMALS}`,
      );
    const meanOne = 10 ** meanMintSupply.value.decimals;

    const usdcMintSupply = await this.connection.getTokenSupply(usdcMint);
    if (!usdcMintSupply) throw new Error('USDC MINT not found');
    if (usdcMintSupply.value.decimals !== DECIMALS)
      // USDC_DECIMALS
      throw Error(
        `Unsupported USDC decimals: ${usdcMintSupply.value.decimals}. Only value supported is: ${DECIMALS}`,
      ); // USDC_DECIMALS
    ...
    const authorityMeanTokenResponse =
      await this.connection.getTokenAccountBalance(idoAuthorityMean);
    const authorityMeanTokenAmount = new BN(
      authorityMeanTokenResponse.value.amount ?? 0,
    );
    if (authorityMeanTokenAmount.lt(idoMeanAmountBn)) {
      throw Error('Insufficient MEAN balance');
    }

    const [idoWithdrawals, createWithdrawalsTx] =
      await this.createCreateWithdrawalsTx(idoAuthority);
  ...
  public async createDepositUsdcTx(
    ...
    if (!currentUserPubKey) throw new Error('Must connect wallet first');
    ...
    if (idoAccount === null) throw new Error('IDO account not found');
    ...
    const [userIdo, userIdoBump] = await this.findUserIdoProgramAddress(
      currentUserPubKey,
      meanIdoAddress,
    );

    const usdcAmountBn = new anchor.BN(amount * 10 ** DECIMALS);
    const userUsdcTokenResponse =
      await program.provider.connection.getTokenAccountBalance(userUsdcAddress);
  ...
  public async createWithdrawUsdcTx(
    ...
    const withdrawUsdcTx = program.transaction.withdrawUsdc(usdcAmountBn, {
      accounts: {
        userAuthority: currentUserPubKey,
        userUsdc: userUsdcAddress,
        userIdo: userIdo,
  ...
  public async createWithdrawMeanLpTx(
    ...
    const [userIdo] = await this.findUserIdoProgramAddress(
      currentUserPubKey,
      meanIdoAddress,
    );
  ...
  public async createCreateWithdrawalsTx(
    ...
    if (!idoAuthorityPubkey) throw new Error('Must connect wallet first');
    ...
    const createWithdrawalsTx = program.transaction.createIdoWithdrawals({
      ...
      instructions: [
        await program.account.idoWithdrawals.createInstruction(
          withdrawalsKeypair,
        ),
      ],
  ...
  public async createUpdateTx(
    ...
    if (!idoAuthorityAddress) throw new Error('Must connect wallet first');
    ...
    if (this.verbose) {
      ...
      console.log(
        ` lastContUsdcContributedBefore:    ${lastContUsdcContributedBefore.toString()}`,
      );
      console.log();
    }
  ...
  public async createReddemTx(idoAddress: PublicKey): Promise<Transaction> {
    const currentUserPubKey = this.userPubKey;
    if (!currentUserPubKey) throw new Error('Must connect wallet first');
    const userWallet = IdoClient.createReadonlyWallet(currentUserPubKey);
    const program = IdoClient.createProgram(
    ...
    const idoAccount = await program.account.idoAccount.fetchNullable(
      idoAddress,
    );
    if (idoAccount === null) throw new Error('IDO account not found');
    ...
    const userIdoAccount = await program.account.userIdoAccount.fetchNullable(
      userIdo,
    );
    if (userIdoAccount === null) throw new Error('User contribution not found');
  ...
  public async listIdos(
    stortByStartTs = true,
    desc = true,
  ): Promise<Array<IdoDetails>> {
    if (!this.userPubKey) throw new Error('Must connect wallet first');

    const ddcaAccounts = await this.readonlyProgram.account.idoAccount.all(
  ...
  public async getIdo(meanIdoAddress: PublicKey): Promise<IdoDetails | null> {
    const idoAccount =
      await this.readonlyProgram.account.idoAccount.fetchNullable(
        meanIdoAddress,
      );
    if (idoAccount === null) return null;
  ...
  public async getUserIdo(
    idoPubKey: PublicKey,
    userIdoPubKey: PublicKey | undefined | null,
  ): Promise<UserIdoDetails | null> {
    if (!this.userPubKey) {
      return null;
    }

    if (!userIdoPubKey) {
      [userIdoPubKey] = await this.findUserIdoProgramAddress(
        this.userPubKey,
        idoPubKey,
      );
    }

    const userIdoAccount =
      await this.readonlyProgram.account.userIdoAccount.fetchNullable(
        userIdoPubKey,
      );
    if (userIdoAccount === null) return null;
  ...
  public async getIdoStatus(idoAddress: PublicKey): Promise<IdoStatus> {
    ...
    const currentIdoStatus: IdoStatus = {
      ...
      currentMaxUsdcContributionTokenAmount:
        statusEvent.usdcPerUserMaxCurrent.toNumber(),
      ...
      usdcTotalContributedTokenAmount:
        statusEvent.usdcTotalContributed.toNumber(),
      ...
      gaTotalUsdcContributedTokenAmount:
        statusEvent.gaUsdcTotalContributed.toNumber(),
      gaMeanTotalPurchased: toUiAmount(statusEvent.gaMeanTotalPurchased),
      gaMeanTotalPurchasedTokenAmount:
        statusEvent.gaMeanTotalPurchased.toNumber(),
      gaIsOpen: statusEvent.gaIsOpen as boolean,
      currentImpliedMeanPrice: toUiAmount(statusEvent.meanImpliedPrice),
      currentImpliedMeanPriceTokenAmount:
        statusEvent.meanImpliedPrice.toNumber(),
      finalMeanPrice: toUiAmount(statusEvent.meanImpliedPrice),
      finalMeanPriceTokenAmount: statusEvent.meanImpliedPrice.toNumber(),
      finalMeanPurchasedAmount: toUiAmount(
        BN.min(statusEvent.gaMeanTotalPurchased, statusEvent.meanTotalMax),
      ),
      finalMeanPurchasedTokenAmount: BN.min(
        statusEvent.gaMeanTotalPurchased,
        statusEvent.meanTotalMax,
      ).toNumber(),
    ...
    if (this.userPubKey) {
      ...
      if (userIdoAccount) {
        ...
        const idoWithdrawals =
          await this.readonlyProgram.account.idoWithdrawals.fetchNullable(
            statusEvent.idoWithdrawalsAddress,
          );
        ...
        if (idoWithdrawals) {
          ...
          for (let i = 0; i < lastIndex; i++) {
            ...
            if (wtw.contributorNumber < userIdoAccount.contributorNumber) {
              const withdrawalAmount = (idoWithdrawals.withdrawals as any[])[i]
                .amount as BN;
              netUsdcWithdrewBeforeUser =
                netUsdcWithdrewBeforeUser.add(withdrawalAmount);
            }
          }

          const usdcTotalContributedBeforeUser =
            userIdoAccount.usdcNetDepositsSnapshot.sub(
              new BN(netUsdcWithdrewBeforeUser),
            );

          if (usdcTotalContributedBeforeUser.gte(statusEvent.usdcTotalMax)) {
            userUsdcInGa = new BN(0);
          } else {
            const diff = statusEvent.usdcTotalMax.sub(
              usdcTotalContributedBeforeUser,
            );
        ...
        currentIdoStatus.userContributorNumber =
          userIdoAccount.contributorNumber;
        currentIdoStatus.userHasContributed = true;
        currentIdoStatus.userContributionTs =
          userIdoAccount.contributionTs.toNumber();
        currentIdoStatus.userContributionUpdatedTs =
          userIdoAccount.contributionUpdatedTs.toNumber();
        currentIdoStatus.userUsdcContributedAmount = toUiAmount(
          userIdoAccount.usdcContributedAmount,
        );
        currentIdoStatus.userUsdcContributedTokenAmount =
          userIdoAccount.usdcContributedAmount.toNumber();
        currentIdoStatus.userMeanPurchasedAmount = toUiAmount(
          userIdoAccount.meanPurchasedAmount,
        );
        currentIdoStatus.userMeanPurchasedTokenAmount =
          userIdoAccount.meanPurchasedAmount.toNumber();
        currentIdoStatus.userMeanImpliedAmount = toUiAmount(
          userImpliedMeanTokenAmount,
        );
        currentIdoStatus.userMeanImpliedTokenAmount =
          userImpliedMeanTokenAmount.toNumber();
  ...
  public toString(): string {
    return `{ rpcUrl: ${
      this.rpcUrl
    }, ownerAccountAddress: ${this.userPubKey?.toBase58()}, commitment: ${
      this.readonlyProvider?.opts?.commitment
    }, preflightCommitment: ${
      this.readonlyProvider?.opts?.preflightCommitment
    }, skipPreflight: ${this.readonlyProvider?.opts?.skipPreflight} }`;
  }

  private async getRpcVersion(): Promise<anchor.web3.Version> {
    if (!this.rpcVersion) this.rpcVersion = await this.connection.getVersion();
  ...
  public async startTracking(
    idoPubkey: PublicKey,
    idoStatusChangedCallback: (status: IdoStatus) => void,
  ): Promise<void> {
    if (this.idoTracker) throw new Error('Already tracking');

    const currentIdoStatus = await this.getIdoStatus(idoPubkey);
    const clusterTimeOffsetInSeconds =
      currentIdoStatus.clusterTs - Math.floor(Date.now() / 1000);
    // console.log("clusterTimeOffsetInSeconds", clusterTimeOffsetInSeconds)

    if (this.verbose) console.log('subscribe ido:', idoPubkey.toBase58());
    const idoEventEmitter =
      this.readonlyProgram.account.idoAccount.subscribe(idoPubkey);

    const idoTracker = new IdoTracker(
      idoPubkey,
      clusterTimeOffsetInSeconds,
      idoEventEmitter,
      null,
      idoStatusChangedCallback,
      this.verbose,
    );
    ...
    if (!idoDetails) throw new Error(`IDO ${idoPubkey} not found`);
    if (!idoTracker.latestIdo) idoTracker.latestIdo = idoDetails;

    if (this.userPubKey) {
      const [userIdoPubKey] = await this.findUserIdoProgramAddress(
        this.userPubKey,
        idoPubkey,
      );
      idoTracker.userIdoPubKey = userIdoPubKey;
      if (this.verbose) console.log('userIdo:', userIdoPubKey.toBase58());

      if (this.verbose)
        console.log('subscribe user ido:', userIdoPubKey.toBase58());
      const userIdoEventEmitter =
        this.readonlyProgram.account.userIdoAccount.subscribe(userIdoPubKey);
      ...
      userIdoEventEmitter.on('change', userdoAccount => {
        idoTracker.latestUserIdo = mapUserIdoDetails(
          userIdoPubKey,
          userdoAccount,
        ); //*
      });
      ...
      if (!idoTracker.latestUserIdo) idoTracker.latestUserIdo = userIdoDetails;
    }
  ...
  public stopTracking() {
    if (!this.idoTracker) return;

    if (this.verbose) {
      console.log('unsubscribe ido:', this.idoTracker.idoPubKey.toBase58());
    }
    this.readonlyProgram.account.idoAccount.unsubscribe(
      new PublicKey(this.idoTracker.idoPubKey),
    );

    if (this.userPubKey && this.idoTracker.userIdoPubKey) {
      if (this.verbose)
        console.log(
          'unsubscribe user ido:',
          this.idoTracker.userIdoPubKey.toBase58(),
        );
      this.readonlyProgram.account.userIdoAccount.unsubscribe(
        this.idoTracker.userIdoPubKey,
      );
    }
...
class IdoTracker {
  ...
  public getIdoStatus(): IdoStatus {
    if (!this.latestIdo) throw new Error('ido is not set');

    const estimatedClusterTimeTs =
      Math.floor(Date.now() / 1000) + this.clusterTimeOffsetInSeconds;
    const t = new BN(estimatedClusterTimeTs).sub(
      new BN(this.latestIdo.idoStartTs),
    );
    const status: IdoStatus = {
      ...
      usdcNetWithdrawalsTokenAmount:
        this.latestIdo.usdcTotalWithdrawalsTokenAmount,
      ...
      usdcTotalContributedTokenAmount:
        this.latestIdo.usdcTotalCurrentTokenAmount,
      ...
      gaTotalUsdcContributedTokenAmount:
        this.latestIdo.gaUsdcTotalContributedTokenAmount,
      gaMeanTotalPurchased: this.latestIdo.gaMeanTotalPurchased,
      gaMeanTotalPurchasedTokenAmount:
        this.latestIdo.gaMeanTotalPurchasedTokenAmount,
      gaIsOpen: this.latestIdo.gaIsOpen,
      currentImpliedMeanPrice: this.latestIdo.meanImpliedPrice,
      currentImpliedMeanPriceTokenAmount:
        this.latestIdo.meanImpliedPriceTokenAmount,
      finalMeanPrice: this.latestIdo.meanImpliedPrice,
      finalMeanPriceTokenAmount: this.latestIdo.meanImpliedPriceTokenAmount,
      finalMeanPurchasedAmount: Math.min(
        this.latestIdo.gaMeanTotalPurchased,
        this.latestIdo.meanTotalMax,
      ),
      finalMeanPurchasedTokenAmount: Math.min(
        this.latestIdo.gaMeanTotalPurchasedTokenAmount,
        this.latestIdo.meanTotalMaxTokenAmount,
      ),
    ...
    if (this.latestUserIdo) {
      ...
      status.userContributionUpdatedTs =
        this.latestUserIdo.contributionUpdatedTs;
      status.userUsdcContributedAmount =
        this.latestUserIdo.usdcContributedAmount;
      status.userUsdcContributedTokenAmount =
        this.latestUserIdo.usdcContributedTokenAmount;
      status.userMeanPurchasedAmount = this.latestUserIdo.meanPurchasedAmount;
      status.userMeanPurchasedTokenAmount =
        this.latestUserIdo.meanPurchasedTokenAmount;
    ...
    if (t.lt(new BN(0))) {
      ...
      status.currentMeanPriceTokenAmount =
        this.latestIdo.meanPriceStartTokenAmount;
      status.currentMaxUsdcContribution = this.latestIdo.usdcPerUserMaxStart;
      status.currentMaxUsdcContributionTokenAmount =
        this.latestIdo.usdcPerUserMaxStartTokenAmount;
    }
    // after end
      ...
      status.currentMeanPriceTokenAmount =
        this.latestIdo.meanPriceEndTokenAmount;
      status.currentMaxUsdcContribution = this.latestIdo.usdcPerUserMaxEnd;
      status.currentMaxUsdcContributionTokenAmount =
        this.latestIdo.usdcPerUserMaxEndTokenAmount;
    }
    // running
    else {
      // if (this.verbose)
      ...
      const refreshIntervalBn = new BN(
        this.latestIdo.curveRefreshIntervalInSeconds,
      );
      ...
      status.currentMaxUsdcContribution =
        currentMaxUsdcPerUser.toNumber() / 10 ** DECIMALS;
      status.currentMaxUsdcContributionTokenAmount =
        currentMaxUsdcPerUser.toNumber();
    }
...
export function meanPriceCurve(ps: BN, pe: BN, t_total: BN, t: BN): BN {
  ...
  const numerator = uDelta.add(new BN(2).mul(kFactor)).mul(tSub).mul(uDelta);
  const denominator = new BN(4)
    .mul(t_total)
    .mul(kFactor)
    .add(new BN(2).mul(uDelta).mul(tSub));
...
export function mapIdoDetails(
  idoAddress: string,
  idoAccountUntyped: any,
): IdoDetails {
  ...
  return {
    ...
    usdcPerUserMaxStart:
      idoAccount.usdcPerUserMaxStart.toNumber() / 10 ** DECIMALS, // USDC_DECIMALS
    ...
    curveRefreshIntervalInSeconds:
      idoAccount.curveRefreshIntervalInSeconds.toNumber(),
    ...
    usdcTotalWithdrawals:
      idoAccount.usdcNetWithdrawals.toNumber() / 10 ** DECIMALS, // MEAN_DECIMALS
    usdcTotalWithdrawalsTokenAmount: idoAccount.usdcNetWithdrawals.toNumber(),
    usdcTotalCurrent:
      idoAccount.usdcTotalContributed.toNumber() / 10 ** DECIMALS, // USDC_DECIMALS
    ...
    gaUsdcTotalContributed:
      idoAccount.gaUsdcTotalContributed.toNumber() / 10 ** DECIMALS, // MEAN_DECIMALS
    gaUsdcTotalContributedTokenAmount:
      idoAccount.gaUsdcTotalContributed.toNumber(),
    // gaTotalContributors: idoAccount.gaTotalContributors,
    gaMeanTotalPurchased:
      idoAccount.gaMeanTotalPurchased.toNumber() / 10 ** DECIMALS, // MEAN_DECIMALS
    ...
    gaLastContributorUsdcContributedBefore:
      idoAccount.gaLastContributorUsdcContributedBefore.toNumber(),
    ...
    idoDurationInSeconds:
      (idoAccount.idoTimes as IdoTimes).idoEndTs.toNumber() -
      idoTimes.idoStartTs.toNumber(),
  };
}

export function mapUserIdoDetails(
  userIdoPubKey: PublicKey,
  userIdoAccount: userIdoAccountType,
): UserIdoDetails {
  return {
    ...
    usdcNetDepositsSnapshotTokenAmount:
      userIdoAccount.usdcNetDepositsSnapshot.toNumber(),
    usdcNetWithdrawalsSnapshot: toUiAmount(
      userIdoAccount.usdcNetWithdrawalsSnapshot,
    ),
    usdcNetWithdrawalsSnapshotTokenAmount:
      userIdoAccount.usdcNetWithdrawalsSnapshot.toNumber(),
    ...
    meanPriceAtContributionTs: toUiAmount(
      userIdoAccount.meanPriceAtContributionTs,
    ),
    meanPriceAtContributionTsTokenAmount:
      userIdoAccount.meanPriceAtContributionTs.toNumber(),
