...
export const VestingContractCreateStreamModal = (props: {
  ...
  const isFeePaidByTreasurer = useMemo(() => {
    return streamTemplate ? streamTemplate.feePayedByTreasurer : false;
  }, [streamTemplate]);
  ...
  const getMinBalanceRequired = useCallback(() => {
    ...
    return minRequired > MIN_SOL_BALANCE_REQUIRED
      ? minRequired
  ...
  const getStreamTxConfirmDescription = (multisig: string) => {
    ...
    let message = '';
    if (cliff === '--') {
      message = `Create stream to send ${rate} over ${lockPeriodAmount} ${getLockPeriodOptionLabel(
        lockPeriodFrequency,
        t,
      )}.`;
    } else {
      message = `Create stream to send ${rate} over ${lockPeriodAmount} ${getLockPeriodOptionLabel(
        lockPeriodFrequency,
        t,
      )} with ${cliff} released on commencement.`;
    }
    return message;
  };
  ...
  const getStreamTxConfirmedDescription = (multisig: string) => {
    ...
    let message = '';
    if (cliff === '--') {
      message = `Stream to send ${rate} over ${lockPeriodAmount} ${getLockPeriodOptionLabel(
        lockPeriodFrequency,
        t,
      )} has been ${multisig ? 'proposed' : 'scheduled'}.`;
    } else {
      message = `Stream to send ${rate} over ${lockPeriodAmount} ${getLockPeriodOptionLabel(
        lockPeriodFrequency,
        t,
      )} with ${cliff} released on commencement has been ${
        multisig ? 'proposed' : 'scheduled'
      }.`;
    }
    return message;
  };
    ...
    <Modal
      ...
      title={
        <div className="modal-title">
          {t('vesting.create-stream.modal-title')}
        </div>
      }
      footer={null}
      open={isVisible}
    ...
    >
      <div className="scrollable-content">
        ...
        <div className={`panel1 ${currentStep === 0 ? 'show' : 'hide'}`}>
          ...
          {/* Vesting Stream name */}
          <div className="form-label">
            {t('vesting.create-stream.vesting-stream-name-label')}
          </div>
          <div className="well">
            <div className="flex-fixed-right">
              <div className="left">
                <input
                  ...
                  placeholder={t(
                    'vesting.create-stream.vesting-stream-name-placeholder',
                  )}
                  spellCheck="false"
          ...
          {/* Beneficiary address */}
          <div className="form-label">
            {t('vesting.create-stream.beneficiary-address-label')}
          </div>
          <div className="well">
            <div className="flex-fixed-right">
              <div className="left position-relative">
                <span className="recipient-field-wrapper">
                  <input
                    ...
                    placeholder={t(
                      'vesting.create-stream.beneficiary-address-placeholder',
                    )}
                    required={true}
                    spellCheck="false"
          ...
          {treasuryOption === TreasuryType.Open ? (
            ...
            <div className="form-label">
              {t('vesting.create-stream.total-funds-to-commit')}
            </div>
          )}
        ...
        <div className={`panel2 ${currentStep === 1 ? 'show' : 'hide'}`}>
          ...
          <div className="flex-fixed-right">
            <div className="left">
              <h2 className="form-group-label">
                {t('vesting.create-stream.step-two-label')}
              </h2>
            </div>
            <div className="right">
              <span
          ...
          <div className="px-1 font-size-100 font-bold">
            {vestingStreamName ? vestingStreamName : '--'}
          </div>
          <div
            className={`mb-2 px-1 ${
              isXsDevice ? 'font-size-80' : 'font-size-90'
            }`}
          >
          ...
          <div className="ml-1">
            ...
            <InfoIcon
              content={
                <span>
                  {t(
                    'vesting.create-stream.verified-recipient-disclaimer-tooltip',
                  )}
                </span>
              }
              placement="top"
          ...
          {/* CTAs */}
          <div
            className={`two-column-form-layout mt-3${
              isXsDevice ? ' reverse' : ''
            }`}
          >
