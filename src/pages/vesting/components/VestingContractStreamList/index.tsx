...
export const VestingContractStreamList = (props: {
  ...
  const [retryCloseOperationPayload, setRetryCloseOperationPayload] = useState<
    VestingContractCloseStreamOptions | undefined
  >(undefined);
  ...
  const copyAddressToClipboard = useCallback(
    // biome-ignore lint/suspicious/noExplicitAny: Anything can be received
    (address: any) => {
      if (!address) {
        return;
      }
        ...
        switch (item.statusCode) {
          case STREAM_STATUS_CODE.Scheduled:
            return t('streams.status.scheduled', {
              date: getShortDate(item.startUtc),
            });
          case STREAM_STATUS_CODE.Paused:
            if (item.isManuallyPaused) {
  ...
  }, [streamTemplate, vestingContract]);
  ...
  const showVestingContractStreamDetailModal = useCallback(
    () => setIsVestingContractStreamDetailModalVisibility(true),
    [],
  );
  ...
  const onAcceptCloseStream = (options: VestingContractCloseStreamOptions | undefined) => {
  ...
  const onExecuteCloseStreamTransaction = async (closeStreamOptions: VestingContractCloseStreamOptions | undefined) => {
    ...
    if (!closeStreamOptions) {
      return;
    }

    setTransactionCancelled(false);
    setOngoingOperation(OperationType.StreamClose);
    setRetryCloseOperationPayload(closeStreamOptions);
    setIsBusy(true);

    const closeStream = async (data: {
      stream: string;
      payer: string;
      ...
      if (!vestingContract || !multisigClient || !multisigAccounts || !publicKey) {
        return null;
      }

      const treasury = vestingContract;
      const multisig = multisigAccounts.filter(m => m.authority.equals(treasury.owner))[0];
    ...
    const createTx = async (): Promise<boolean> => {
      if (!publicKey || !highlightedStream || !tokenStreamingV2 || !selectedToken) {
        transactionLog.push({
        ...
        if (sign.encodedTransaction) {
          ...
          if (sent.signature && !transactionCancelled) {
            ...
            enqueueTransactionConfirmation({
              ...
              extras: {
                vestingContractId: vestingContract.id.toBase58(),
  ...
  const getStreamClosureMessage = () => {
    ...
    if (publicKey && highlightedStream) {
      ...
      if (isDateInTheFuture(paymentStartDate)) {
        ...
        message = t('vesting.close-account.close-stream-finished', {
  ...
  useEffect(() => {
    if (!streamList || !tokenStreamingV2) {
      return;
    }

    const timeout = setTimeout(() => {
      tokenStreamingV2.refreshStreams(streamList || []).then(streams => {
  ...
  const renderStreamOptions = (item: Stream) => {
    ...
    if (
      ...
      (vestingContract.accountType === AccountType.Lock && item.statusCode !== STREAM_STATUS_CODE.Running)
    ) {
      //
      items.push({
        key: '01-close-stream',
    ...
    items.push({
      ...
      label: (
        <div onKeyDown={() => {}} onClick={() => copyAddressToClipboard(item.id)}>
            ...
            <div className='row two-col-ctas mt-3'>
              <div className='col-6'>
                <Button
                  ...
                  onClick={() =>
                    ongoingOperation === OperationType.StreamClose
                      ? onExecuteCloseStreamTransaction(retryCloseOperationPayload)
                      : hideTransactionExecutionModal()
                  }
                >
                  {t('general.retry')}
              ...
              return (
                <div
                  ...
                  className={`transaction-list-row stripped-rows ${
                    isDeletedStream(item.id.toBase58()) ? 'disabled blurry-1x' : getSelectedClass()
                  }`}
                >
                  <div
